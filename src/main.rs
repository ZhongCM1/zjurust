...
fn main() {
    ...
    //增加"-v"参数，输出目标目录下的所有文件
    let mut flag = 0;
    if args.len() > 3 && args[3] == "-v" {
        flag = 1;
    }
    if flag == 1 {
        println!("以下是目标目录下的所有文件：");
    }
    match find(&args[1], &regex, flag) {
        Ok(matches) => {
            if matches.is_empty() {
                println!("未找到匹配项。");
            } else {
...
fn find<P: AsRef<Path>>(root: P, regex: &Regex, flag: i32) -> Result<Vec<String>, Box<dyn std::error::Error>>{
